...
interface AdContentStepProps {
  ...
  updateData: (data: Partial<AdContentStepProps['data']>) => void;
}
      ...
      <div className="space-y-8">
        ...
        {!data.autoGenerateText && (
          <div>
            ...
            <Textarea
              id="custom-text"
              placeholder={`Ej: ¡Descubre ${businessData.name}! ${businessData.description.slice(0, 50)}...`}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors">
                <Image className="h-8 w-8 text-gray-400 mb-2" />
                <p className="text-sm text-gray-600 mb-2">Sube una imagen o deja que nosotros te recomendemos una</p>
                <Button 
                  variant="outline" 
                  size="sm" 
              ...
              {previewUrl ? (
                ...
                <div className="w-full h-[180px] bg-gray-200 rounded flex items-center justify-center">
                  <p className="text-sm text-gray-500">
                    {data.autoGenerateText ? "Se generará una imagen automáticamente" : "Vista previa de la imagen"}
                  </p>
                </div>
              )}
            </div>
        ...
        <div>
          <Label className="mb-2 block">¿A dónde quieres que lleve el anuncio?</Label>
          <RadioGroup 
            value={data.destinationType} 
            onValueChange={(value) => updateData({ destinationType: value })}
            className="grid grid-cols-1 md:grid-cols-2 gap-4"
          >
            <div className={`border rounded-lg p-4 cursor-pointer transition-all ${data.destinationType === 'website' ? 'border-primary bg-blue-50' : 'hover:border-gray-300'}`}>
            ...
            <div className={`border rounded-lg p-4 cursor-pointer transition-all ${data.destinationType === 'whatsapp' ? 'border-primary bg-blue-50' : 'hover:border-gray-300'}`}>
          ...
          <div className="mt-4">
            <Label htmlFor="destination-url">
              {data.destinationType === 'website' ? 'URL de la página web' : 'Número de WhatsApp (con código de país)'}
            </Label>
            <Input
              id="destination-url"
              placeholder={data.destinationType === 'website' ? 'https://www.tusitio.com' : '+5491112345678'}
        ...
        <div>
          ...
          >
            <div className={`border rounded-lg p-3 text-center cursor-pointer transition-all ${data.cta === 'Más info' ? 'border-primary bg-blue-50' : 'hover:border-gray-300'}`}>
            ...
            <div className={`border rounded-lg p-3 text-center cursor-pointer transition-all ${data.cta === 'Comprar ahora' ? 'border-primary bg-blue-50' : 'hover:border-gray-300'}`}>
            ...
            <div className={`border rounded-lg p-3 text-center cursor-pointer transition-all ${data.cta === 'Contactar' ? 'border-primary bg-blue-50' : 'hover:border-gray-300'}`}>
            ...
            <div className={`border rounded-lg p-3 text-center cursor-pointer transition-all ${data.cta === 'Registrarse' ? 'border-primary bg-blue-50' : 'hover:border-gray-300'}`}>
